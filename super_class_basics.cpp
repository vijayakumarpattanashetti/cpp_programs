...
class laptopCls { // define class
    ...
    laptopCls (string processorVar, string RAM_Var, string harddiskVar, 
        ...
        cout << "I am a constructor.\n"; // It is a default function which will be executed when the class is initiated
    ...
    ~laptopCls(){
        cout << "I am a destructor.\n"; // It is a default function which will be executed at the end of the class
    }
};

class alienWareLaptopCls : public laptopCls {
    public:
    string GPU; //nvidia rtx 2080

    alienWareLaptopCls  (string processorVar, string RAM_Var, string harddiskVar, 
                    float screensizeVar, bool touchscreenVar, string brandVar,
            string GPU_Var) : laptopCls(processorVar, RAM_Var, harddiskVar, screensizeVar, touchscreenVar, brandVar) {
        GPU = GPU_Var;
    }

    void dispInfoFunc() { // function to print
        laptopCls :: dispInfoFunc();
        cout << "GPU \t: " << GPU << endl << endl;
        cout << laptopCls :: getModelFunc() << endl;
    }
};
